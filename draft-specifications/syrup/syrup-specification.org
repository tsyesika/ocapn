...
Dictionaries are sorted very simply.
All keys are first /fully serialized/ and then sorted by the encoded-bytes
mechanism.
See [[*Bytewise sorting]] for details.
...
Beginning with fully encoded bytestrings
 
 - Define =bstr1_len= as the 
 - Begin looping with =pos= set to =0=.
 - 
